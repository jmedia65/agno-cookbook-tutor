...
In the [previous chapter](02_agent_.html), we met the **[Agent](02_agent_.html)**, our specialized AI assistant built around a powerful **[Model](01_model_.html)**. We learned how to give Agents instructions to guide their behavior.

But what if our Agent needs information that wasn't in its training data? What if it needs to know the _current_ stock price of Apple, search the web for today's news, or perform a complex calculation? By default, an [Agent](02_agent_.html)'s knowledge is frozen at the time its [Model](01_model_.html) was trained. It can't access real-time information or perform actions outside of generating text.

This is where **Tools** come in!

## What are Tools?

Imagine you hire a very smart assistant ([Agent](02_agent_.html)) with a great brain ([Model](01_model_.html)). They can write, analyze, and talk about many things based on their general knowledge. But if you ask them for the current weather, the result of a complex calculation, or information from a specific database, they'll need some equipment to find the answer.
...
By equipping an [Agent](02_agent_.html) with Tools, you empower it to go beyond its pre-trained knowledge and perform tasks that require real-time data, calculations, or interactions with other systems.
...
- `ReasoningTools`: Helps the agent think step-by-step (more on this in the [next chapter](04_reasoning___thinking_tools_.html)!).
...
1.  **Import:** We import `Agent`, `Claude` (our [Model](01_model_.html)), and crucially, `YFinanceTools` from `agno.tools.yfinance`.
...
4.  **Run:** We ask the Agent a question ("What is the stock price of Apple?"). The Agent, guided by its [Model](01_model_.html), realizes it needs external, real-time data. It sees it has the `YFinanceTools` available. It uses the tool to fetch the current price from Yahoo Finance and then incorporates that information into its final answer.
...
Here, the `finance_agent` is equipped with a `YFinanceTools` instance configured to fetch stock prices, analyst recommendations, _and_ company info. The [Agent](02_agent_.html) will automatically figure out which part of the tool to use based on the question asked.
...
2.  **Agent -> Model:** The Agent sends your request, its instructions, and a list of its available `tools` (like `YFinanceTools` with its capabilities) to its assigned [Model](01_model_.html) (e.g., Claude).
3.  **Model Decides:** The [Model](01_model_.html) analyzes the request and the available tools. It decides if a tool is needed and, if so, which one and how to call it (e.g., "Call `YFinanceTools.get_stock_price` with the ticker `AAPL`").
4.  **Model -> Agent:** The [Model](01_model_.html) tells the Agent which tool function to execute and what arguments to use.
...
9.  **Agent -> Model (Again):** The Agent sends the result obtained from the tool back to the [Model](01_model_.html), along with the original question, asking it to formulate the final answer.
10. **Model -> Agent:** The [Model](01_model_.html) generates the final, human-readable response incorporating the tool's result (e.g., "The current stock price for Apple (AAPL) is $214.29 USD.").
...
Agno handles the complex parts of this process: letting the [Model](01_model_.html) know which tools are available, executing the chosen tool function, and feeding the results back into the conversation. You just need to provide the tools!

## Conclusion

You've learned how **Tools** give your Agno **[Agents](02_agent_.html)** superpowers! They allow Agents to break free from the limitations of their training data and interact with the real world.

Key Takeaways:

- Tools enable Agents to perform actions like searching the web, fetching real-time data, doing calculations, or running code.
- You equip an [Agent](02_agent_.html) with tools by passing a list of tool instances to the `tools=` parameter during initialization.
- Agents can use multiple tools, and the underlying [Model](01_model_.html) intelligently decides which tool to use based on the task.
...
**[Next Chapter: Reasoning / Thinking Tools](04_reasoning___thinking_tools_.html)**
