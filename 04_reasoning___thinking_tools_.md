...
Welcome back! In the [previous chapter](03_tools_.html), we learned how standard **[Tools](03_tools_.html)** give our **[Agents](02_agent_.html)** superpowers to interact with the outside world, like searching the web or getting stock prices.

But what happens when an [Agent](02_agent_.html) needs to tackle a really complex task? Something that requires planning, breaking down steps, or thinking through different options before giving a final answer?

Imagine asking an assistant to "Write a comprehensive report comparing NVIDIA and AMD, including their latest financial performance, product lines, and future outlook." A simple [Agent](02_agent_.html) might just try to answer directly, potentially missing key steps or structure. Wouldn't it be better if the assistant could first _plan_ its approach?

This is where **Reasoning / Thinking Tools** come in!

## What are Reasoning / Thinking Tools?

Think of Reasoning/Thinking tools as giving your [Agent](02_agent_.html) the ability to **"think out loud"** or **"create a draft plan"** before committing to a final action or response.

It's a specialized type of [Tool](03_tools_.html) that doesn't necessarily interact with the _outside_ world, but rather helps the [Agent](02_agent_.html) structure its _internal_ thought process.

Using these tools, an [Agent](02_agent_.html) can:
...
Let's equip an [Agent](02_agent_.html) with the ability to reason. We'll adapt the `reasoning_finance_agent.py` example. Our agent needs to write a report on NVIDIA (NVDA), which involves multiple steps (getting info, news, recommendations, then structuring the report).
...
2.  **Add Tool:** Just like any other [Tool](03_tools_.html), we add an instance of `ReasoningTools()` to the `tools=` list when creating the `Agent`. The `add_instructions=True` helps guide the [Model](01_model_.html) on how to use the tool effectively.
...
How does adding `ReasoningTools` or `ThinkingTools` actually make the [Agent](02_agent_.html) "think"?

1.  **You -> Agent:** You give the Agent a complex task (e.g., "Write a report on NVDA").
2.  **Agent -> Model:** The Agent sends your request, its instructions, and the list of available tools (including `ReasoningTools` and `YFinanceTools`) to its [Model](01_model_.html) (e.g., Claude).
3.  **Model Decides to Plan:** The [Model](01_model_.html), recognizing the complexity and the availability of `ReasoningTools`, decides that planning is the best first step. It decides to call the `ReasoningTools` (often a specific function within it like `think` or `plan`).
...
6.  **Reasoning Tool -> Model (Prompt to Generate Plan):** The `ReasoningTools` code essentially asks the _same_ [Model](01_model_.html) (or sometimes a specialized reasoning model): "Given the user's request ('Write a report on NVDA') and the available tools (YFinance), create a step-by-step plan."
...
10. **Agent -> Model (Execute Plan):** The Agent, now equipped with the plan, proceeds to execute the steps. It sends the plan and the next step back to the [Model](01_model_.html), asking it to decide which _other_ tool (like `YFinanceTools`) to call for the current step.
11. **(Model -> Agent -> Tool -> ... -> Agent -> Model):** The Agent executes the steps in the plan, potentially calling other tools as needed (like we saw in the [Tools chapter](03_tools_.html)). It might even use the Reasoning Tool again to reflect or adjust the plan mid-way.
...
You've now learned about **Reasoning / Thinking Tools**, a powerful way to enhance your Agno **[Agents](02_agent_.html)**.

Key Takeaways:

- Reasoning/Thinking tools help Agents plan, break down complex tasks, and think step-by-step.
- They act like an internal "scratchpad" or "drafting" process for the [Agent](02_agent_.html).
- You add them like any other [Tool](03_tools_.html) (e.g., `tools=[ReasoningTools(), ...]`).
...
So far, our Agents have a brain ([Model](01_model_.html)), a purpose ([Agent](02_agent_.html)), tools to act ([Tools](03_tools_.html)), and even tools to think ([Reasoning Tools](04_reasoning___thinking_tools_.html)). But what if we want to give them specific documents or data to learn from? That's where **Knowledge** comes in, and we'll explore it next!

**[Next Chapter: Knowledge](05_knowledge_.html)**
