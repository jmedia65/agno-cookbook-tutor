...
Welcome back! In the [previous chapter](07_storage_.html), we learned about **[Storage](07_storage_.html)**, which allows our **[Agents](02_agent_.html)** to save and recall entire conversations, providing long-term persistence. We've built up quite a capable solo [Agent](02_agent_.html) with a [Model](01_model_.html) (brain), [Tools](03_tools_.html) (actions), [Knowledge](05_knowledge_.html) (library), [Memory](06_memory_.html) (notepad), and [Storage](07_storage_.html) (filing cabinet).

But what if a task is too complex or requires too many different skills for a single [Agent](02_agent_.html)? Imagine you need a detailed financial report on a company. This might require:
...
Think of a **Team** as a way to manage a group of specialized **[Agents](02_agent_.html)**, allowing them to work together on a common goal. It's like being the manager of a project team.

- You have **members**, which are just the [Agents](02_agent_.html) we've already learned about, each potentially having its own role, [Model](01_model_.html), [Tools](03_tools_.html), and [Knowledge](05_knowledge_.html).
...
A Team can operate in different modes, controlling how the member [Agents](02_agent_.html) interact:

1.  **`coordinate` mode:**

    - **Analogy:** A project manager assigning tasks.
    - **How it works:** The Team acts like a leader or manager. It receives the main task, decides which member [Agent](02_agent_.html) is best suited for each sub-task, delegates the work, gathers the results, and compiles the final answer. The leader might even use its own [Tools](03_tools_.html), like [Reasoning / Thinking Tools](04_reasoning___thinking_tools_.html), to plan the work.
...
2.  **`collaborate` mode:**
    ...
    - **How it works:** Member [Agents](02_agent_.html) discuss the task amongst themselves, share ideas, and build upon each other's contributions until a consensus or final result is reached. The Team facilitates this discussion.
...
3.  **`route` mode:**
    ...
    - **How it works:** The Team acts as a smart router. It receives the user's request, analyzes it, and directs it to the _single_ most appropriate member [Agent](02_agent_.html) to handle the entire task.
...
1.  **Import:** We import `Agent`, `Team`, the [Model](01_model_.html) (`OpenAIChat`), and the necessary [Tools](03_tools_.html) (`DuckDuckGoTools`, `YFinanceTools`).
2.  **Define Members:** We create two standard [Agents](02_agent_.html): `web_agent` with web search tools and `finance_agent` with financial tools. Each has its own simple instructions.
3.  **Create Team:**
    - We instantiate `Team`.
...
4.  **Run Task:** We call `report_team.print_response()` with our request. The team leader ([Model](01_model_.html)) receives this, follows its instructions, delegates sub-tasks to `finance_agent` and `web_agent` sequentially, gets their responses back, and finally combines them into the final output printed to the console.
...
2.  **Team Leader -> Leader's Model:** The leader (using its assigned [Model](01_model_.html), e.g., GPT-4o) analyzes the task, its own instructions, and the available `members` (Web Agent, Finance Agent). It might create a plan (possibly using [Reasoning / Thinking Tools](04_reasoning___thinking_tools_.html) if provided).
...
5.  **Member Agent -> Member's Model/Tool:** The member agent executes the sub-task (using its own [Model](01_model_.html) and [Tools](03_tools_.html), e.g., `YFinanceTools`).
...
9.  **Team Leader -> Leader's Model:** Once all sub-tasks are done, the leader sends all collected results to its [Model](01_model_.html) asking it to synthesize the final answer based on the original request.
...
2.  **Team Router -> Router's Model:** The router (using its [Model](01_model_.html)) analyzes the request, its instructions, and the available `members` (English, Spanish, French Agents) to determine the best fit.
...
You've now learned about **Teams** in Agno, a powerful way to orchestrate multiple specialized **[Agents](02_agent_.html)**.

Key Takeaways:

- Teams allow multiple Agents to work together on complex tasks.
- Member agents are just regular [Agents](02_agent_.html) with specific roles, tools, or knowledge.
...
We've covered the core building blocks of Agno: [Model](01_model_.html), [Agent](02_agent_.html), [Tools](03_tools_.html), [Reasoning / Thinking Tools](04_reasoning___thinking_tools_.html), [Knowledge](05_knowledge_.html), [Memory](06_memory_.html), [Storage](07_storage_.html), and now [Team](08_team_.html). How can we put these together to create an interactive web application? Let's find out in the final chapter!

**[Next Chapter: Streamlit Application](09_streamlit_application_.html)**
